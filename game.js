...
let gameState = {
  ...
  flipped:        false, // online rejimda qora o'yinchi uchun true bo'ladi
};
...
function resizeCanvas() {
  if (!canvas) return;
  // .game-board (lokal o'yin) .game-ui padding (20px x2 = 40) + .board-row
  // ichidagi chap/o'ng koordinata ustunlari (18px x2) va gap'lari (3px x2)
  // = 82px joy oladi. #online-canvas esa .online-game-area (padding 16px x2
  // = 32px) ichida, koordinata ustunlarisiz joylashgan — ANIQ shu ikkisini
  // farqlamasak, online taxta kerakidan ancha kichik chizilib qolar edi
  // (mavjud bo'lmagan ustunlar uchun joy behuda "band" qilinardi).
  const isOnline       = canvas.id === 'online-canvas';
  const chrome          = isOnline ? 32 : (40 + (18 + 3) * 2);
  const pagePadding     = chrome + 8;        // kichik qo'shimcha zaxira
  const totalCitadelW   = (CIT_W + CIT_GAP) * 2; // ikkala citadel + bo'shliqlari
  // .online-game-area max-width:600px bilan cheklangan — kengroq
  // ekranlarda ham shu chegaradan oshmasin.
  const containerCap    = isOnline ? (600 - 32) : Infinity;
  const available       = Math.min(window.innerWidth - pagePadding, containerCap) - totalCitadelW;
  const maxCellsWidth   = Math.min(available, 54 * BOARD_COLS);
  CELL_W = Math.max(16, Math.floor(maxCellsWidth / BOARD_COLS));
  CELL_H = CELL_W;
  // Canvas: taxta + 2 citadel + bo'shliqlar
  canvas.width  = CELL_W * BOARD_COLS + totalCitadelW;
...
// ── TAXTANI AYLANTIRISH (online: qora o'yinchi uchun) ───────────
// gameState.flipped === true bo'lsa, taxta 180° aylantirilib chiziladi
// (qora o'yinchi o'z donalarini pastda ko'radi). Bu funksiyalar
// "mantiqiy" (r,c) katak koordinatasini "ekrandagi" (screen) katakka
// o'giradi. Har ikki yo'nalishda ham ishlaydi (o'z-o'ziga teskari).
function flipR(r) { return gameState.flipped ? (BOARD_ROWS - 1 - r) : r; }
function flipC(c) { return gameState.flipped ? (BOARD_COLS - 1 - c) : c; }
...
function buildCoords() {
  // gameState.flipped bo'lsa, harflar va raqamlar teskari tartibda
  // chiziladi — chunki taxtaning o'zi 180° aylantirilgan bo'ladi.
  const baseCols = 'abcdefghijk'.split('');
  const cols = gameState.flipped ? baseCols.slice().reverse() : baseCols;
  ['board-coords-top','board-coords-bottom'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.innerHTML = '';
    // Citadel bo'shlig'i (chap)
    ...
    // Citadel bo'shlig'i (o'ng) — canvas kengligiga aniq mos kelishi
    // uchun (aks holda harflar ustunlar bilan tekislanmay qolardi)
    const spacerEnd = document.createElement('div');
    spacerEnd.style.width = (CIT_W + CIT_GAP) + 'px';
    spacerEnd.style.flex = '0 0 auto';
    el.appendChild(spacerEnd);
  });
  ['board-coords-left','board-coords-right'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.innerHTML = '';
    // BOARD_ROWS = 10, qatorlar: normal holatda 10 (yuqori) → 1 (pastki),
    // flipped holatda 1 (yuqori) → 10 (pastki)
    for (let r = 0; r < BOARD_ROWS; r++) {
      const d = document.createElement('div');
      d.className = 'coord-label';
      d.style.height = CELL_H + 'px';
      d.style.lineHeight = CELL_H + 'px';
      d.textContent = gameState.flipped ? (r + 1) : (BOARD_ROWS - r);
      el.appendChild(d);
    }
  });
}
...
function drawCell(r, c) {
  // (r,c) — mantiqiy (haqiqiy taxtadagi) katak. Ekranda qayerga
  // chizilishini flipR/flipC aniqlaydi (online rejimda qora
  // o'yinchi uchun taxta 180° aylantirilgan bo'lishi mumkin).
  const sr = flipR(r), sc = flipC(c);
  const x = sc * CELL_W, y = sr * CELL_H;
  ctx.fillStyle = (sr + sc) % 2 === 0 ? CLR.light : CLR.dark;
...
function drawCitadels() {
  if (!ctx) return;
  // Normal holatda: Oq citadel chapda (row=1), Qora citadel o'ngda (row=8).
  // gameState.flipped bo'lsa (qora o'yinchi), taxta 180° aylanadi —
  // shuning uchun qaysi citadel chapda/o'ngda ko'rinishi ham almashadi.
  const leftColor  = gameState.flipped ? COLOR.BLACK : COLOR.WHITE;
  const rightColor = gameState.flipped ? COLOR.WHITE : COLOR.BLACK;
  const leftCit     = leftColor  === COLOR.WHITE ? CITADEL.WHITE : CITADEL.BLACK;
  const rightCit    = rightColor === COLOR.WHITE ? CITADEL.WHITE : CITADEL.BLACK;
  const leftOccupant  = leftColor  === COLOR.WHITE ? gameState.whiteCitadel : gameState.blackCitadel;
  const rightOccupant = rightColor === COLOR.WHITE ? gameState.whiteCitadel : gameState.blackCitadel;

  drawOneCitadel(0, flipR(leftCit.r) * CELL_H, leftColor, leftOccupant);
  const rightX = bx() + BOARD_COLS * CELL_W + CIT_GAP;
  drawOneCitadel(rightX, flipR(rightCit.r) * CELL_H, rightColor, rightOccupant);
}
...
function getCellFromEvent(e) {
  ...
  const screenR = Math.floor(rawY  / CELL_H);
  const screenC = Math.floor(boardX / CELL_W);
  // Ekrandagi katakni mantiqiy katakka o'giramiz (flip o'z-o'ziga teskari)
  const r = flipR(screenR);
  const c = flipC(screenC);

  // Citadel zonalari (chap va o'ng panel) — taxta flipped bo'lsa,
  // qaysi citadel qaysi tomonda ko'rinayotgani ham almashadi
  // (drawCitadels dagi mantiqqa mos).
  if (rawX >= 0 && rawX < bx()) {
    const cit = gameState.flipped ? CITADEL.BLACK : CITADEL.WHITE;
    if (screenR === flipR(cit.r)) return { r: cit.r, c: cit.c };
  }
  if (rawX > bx() + BOARD_COLS*CELL_W) {
    const cit = gameState.flipped ? CITADEL.WHITE : CITADEL.BLACK;
    if (screenR === flipR(cit.r)) return { r: cit.r, c: cit.c };
  }
  return { r, c };
}
  ...
  if (isCitadelCoord(tr,tc)) {
    ...
    if (citOwner === piece.color) {
      ...
      if (gameState.mode==='online') {
        ...
        // Faqat MAHALLIY hamlalarda yuboramiz — raqibdan kelgan
        // hamlani qayta serverga jo'natib yubormaslik uchun.
        if (!window.OnlineGame?.state?.applyingRemoteMove) window.onlineOnLocalMoveApplied?.(lastMove);
      } else {
...
// Joriy taxta holatini qaytarish (online.js buni har bir hamladan
// keyin bazaga saqlash uchun chaqiradi — avval bu funksiya umuman
// mavjud bo'lmagan, shu sabab DB dagi board_state doim eskirgan
// (o'yin boshidagi) holatda qolib ketardi).
window.getBoardState = function() {
  return JSON.parse(JSON.stringify(gameState.board));
};